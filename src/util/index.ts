 ...
 */
export const asSeconds = (duration: number): string => `${duration.toFixed(1)}s`;
 ...
 */
export const getFeatureNameByPath = (path: string): string => {
  if (path === '/') return '';
 ...
 */
export const mapValues = (value: number, x1: number, y1: number, x2: number, y2: number) =>
